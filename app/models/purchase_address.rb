class PurchaseAddress
  ...
  with_options presence: true do
    ...
    validates :street
    validates :telephone_number, presence: { message: "can't be blank" }, length: { in: 10..11, message: "is too short" }, format: { with: /\A\d{10,11}\z/, message: "is invalid. Input only number" }
    validates :user_id
    validates :item_id
    validates :token
  end
